    ...
    */
    public func items<
        ...
        return { source in
            ...
            return source.subscribeProxyDataSource(ofObject: self.base, dataSource: dataSource, retainDataSource: true) { [weak RecordBgView = self.base] (_: RxCollectionViewDataSourceProxy, event) -> Void in
                guard let RecordBgView = RecordBgView else {
                    return
                }
                dataSource.collectionView(RecordBgView, observedEvent: event)
            }
        }
    }
}
